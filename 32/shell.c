...
void translate_shell()
{
	...
	if (kstrncmp(keyboard, "rm ",3)) { sh_rm(); return; }
...
void sh_rm()
{
	struct INODE target;
	char* name = &keyboard[3];
	unsigned int inodenum = searchDir(name);

	unsigned int i;

	if ( inodenum == -1 )
	{
		kprintf(" '%s' doesn't exist ",++curline,0,name);
		return;
	}
	else
	{
		findnReadInode(inodenum,&target);

		switch( target.i_mode & 0xF000 )
		{
