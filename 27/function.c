...
void kprintf(char* str, int line, int col,...) // str ���ڸ� line ��° �ٿ� �����ϴ� �Լ�
{
	...
	for (int i = 0; str[i] != 0; i++)
	{
		if ( str[i] == '%' )
		{
			if ( str[i+1] == 'd') kprintf_arg_int(((int)va_arg(va,int)), &video);
			if ( str[i+1] == 's') kprintf_arg_str((char*)va_arg(va,char*), &video);
...
void kprintf_arg_str(char* target, char** video)
{
...
void kprintf_arg_int(int target, char** video)
{
...
int kstrncmp(char* str1, char* str2, int size )
{
